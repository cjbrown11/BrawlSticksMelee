...
namespace BrawlSticksMelee.Sprites
{
    public class PlayerTwo
    {
        ...
        public PlayerTwo(Vector2 startingPosition, StickmanSprite gamePlayer)
        {
        ...
        public void Update(GameTime gameTime, bool poweredUp, ContentManager content)
        {
            ...
            if ((keyboardState.IsKeyDown(Keys.K) && position.X > 0))
            {
                position += new Vector2(-2, 0);
                flipped = true;
            }
            if ((keyboardState.IsKeyDown(Keys.L) && position.X < 670))
            {
                position += new Vector2(2, 0);
                flipped = false;
            }
            if (keyboardState.IsKeyDown(Keys.I) && poweredUp)
            {
                animationFrame = 5;
                blocked = true;
            }
        ...
        public void Draw(GameTime gameTime, SpriteBatch spriteBatch)
        {
            ...
            if (previousKeyboardState.IsKeyUp(Keys.O) && currentKeyboardState.IsKeyDown(Keys.O))
            {
                animationFrame = 2;
                animationTimer = 0.3;
            }
            ...
            {
                if (player.animationFrame == 2 && blocked == false)
                {
                    health -= 1;
                    TakeDamage(1);
                }
